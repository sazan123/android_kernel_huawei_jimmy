...
print_task_at_AEE(struct seq_file *m, struct rq *rq, struct task_struct *p)
{
	...
	if (rq->curr == p) {
#ifdef CONFIG_CGROUP_SCHED
		SEQ_printf_at_AEE(m, "R %15s %5d %9Ld.%06ld %9Ld %5d %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %s %d %ld %d\n",
			p->comm,
			task_pid_nr(p),
			...
			p->exit_state);
#else
		SEQ_printf_at_AEE(m, "R %15s %5d %9Ld.%06ld %9Ld %5d %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %d %ld %d\n",
			p->comm,
			task_pid_nr(p),
			SPLIT_NS(p->se.vruntime),
			(long long)(p->nvcsw + p->nivcsw),
...
#ifdef CONFIG_CGROUP_SCHED
		SEQ_printf_at_AEE(m, "  %15s %5d %9Ld.%06ld %9Ld %5d %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %s %d %ld %d\n",
			p->comm,
			task_pid_nr(p),
			SPLIT_NS(p->se.vruntime),
			(long long)(p->nvcsw + p->nivcsw),
			...
			p->exit_state);
#else
		SEQ_printf_at_AEE(m, "  %15s %5d %9Ld.%06ld %9Ld %5d %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %d %ld %d\n",
			p->comm,
			task_pid_nr(p),
			SPLIT_NS(p->se.vruntime),
			(long long)(p->nvcsw + p->nivcsw),
