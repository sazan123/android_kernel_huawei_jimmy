...
static  char g_Ansemi_charger_ic_name[50]="As2481";
...
#define bq24157_SLAVE_ADDR_WRITE_ERROR   0xDA
...
/**********************************************************
  ...
  *
  *********************************************************/
  static char *g_Ansemi_charger_ic=NULL;
void bq24157_hw_component_detect(void)
{
	...
	if (val == 0x41)
	{
           //g_Ansemi_charger_ic=g_Ansemi_charger_ic_name;
	}
...
char * ontim_get_g_Ansemi_charger_ic(void)
{
	return g_Ansemi_charger_ic;
}
static int bq24157_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        int ret = 0;
	...
	bq24157_hw_component_detect();
        ...
        {
            if(g_Ansemi_charger_ic)
            {
            	    //strcpy(charge_ic_vendor_name,g_Ansemi_charger_ic);
            }
        ...
        {
	    chargin_hw_init_done = KAL_TRUE;
            ret =0;//is_bq24157_exist = 1 //ret = -1;
        }
...
static struct i2c_board_info __initdata i2c_bq24157 = { I2C_BOARD_INFO("bq24157", (bq24157_SLAVE_ADDR_WRITE_ERROR>>1))};
static int __init bq24157_init(void)
{
	...
	if (!node)  i2c_register_board_info(BQ24157_BUSNUM, &i2c_bq24157, 1);
