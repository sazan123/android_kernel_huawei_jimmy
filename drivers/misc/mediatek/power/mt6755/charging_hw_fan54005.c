...
struct dsm_dev fan54005_dsm_dev=
{
	.type=OMTIM_DSM_DEV_TYPE_POWER,
	.id=OMTIM_DSM_DEV_ID_CHARGER,
	.name="Fan54005_charger",
	.buff_size=1024,
};
struct dsm_client *fan54005_dsm_client=NULL;
#endif
//-add by hzb for dsm
...
extern char * ontim_get_g_Second_charger_ic(void);
...
#endif
	battery_log(BAT_LOG_CRTI, "fan54005  charging_hw_init  line=%d\n", __LINE__);

	#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
	fan54005_reg_config_interface(0x06,0x7b); // ISAFE = 1450mA, VSAFE = 4.42V
	#else
	fan54005_reg_config_interface(0x06,0x7b);
	#endif

	fan54005_reg_config_interface(0x00,0xC0); //kick chip watch dog
	//fan54005_reg_config_interface(0x05,0x04);
	if ( !charging_init_flag ) {
		fan54005_reg_config_interface(0x04,0x71); //97mA
		fan54005_reg_config_interface(0x01,0x78);	//TE=1, CE=0, HZ_MODE=0, OPA_MODE=0
		charging_init_flag = KAL_TRUE;
	}
#ifdef CONFIG_ONTIM_DSM
	fan54005_dsm_client=dsm_register_client (&fan54005_dsm_dev);
#endif
...
static int charging_set_cv_voltage(void *data)
{
	...
	register_value =
	    charging_parameter_to_value(FAN54005_VBAT_CV_VTH, GETARRAYNUM(FAN54005_VBAT_CV_VTH), *(u32 *) (data));
	battery_log(BAT_LOG_CRTI, " set CV to %d,  register_value =0x%x\n",*(u32 *) (data),register_value);
...
static int charging_get_current(void *data)
{
	...
	*(u32 *) data = charging_value_to_parameter(FAN54005_CS_VTH, array_size, reg_value);
...
static int charging_set_current(void *data)
{
	...
	if((ontim_get_g_Second_charger_ic())&&(current_value>CHARGE_CURRENT_1000_00_MA))
	{
...
static int charging_set_input_current(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "charging_set_input_current  register_value=%d\n", register_value);
		...
		set_chr_current = bmt_find_closest_level(FAN54005_INPUT_CS_VTH, array_size, *(u32 *) data);
...
static int charging_set_dpm(void *data)
{
   u8 dpm_byte = *(u8 *)data;
   return fan54005_set_vsp(dpm_byte);
}
...
int chr_control_interface(CHARGING_CTRL_CMD cmd, void *data)
{
	...
	if (frist_run)
	{
        ...
        charging_func[CHARGING_CMD_SET_DPM]=charging_set_dpm;
