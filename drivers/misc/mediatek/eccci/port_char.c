...
static int dev_char_open(struct inode *inode, struct file *file)
{
	...
	if (port->rx_ch == CCCI_MD_LOG_RX && port_proxy_get_md_state(port->port_proxy) == READY) {
		...
		if (status_poller == NULL)
			CCCI_NORMAL_LOG(port->md_id, CHAR, "start status poller fail for not found\n");
		else
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	if (port->rx_ch == CCCI_MD_LOG_RX && port_proxy_get_md_state(port->port_proxy) == READY) {
		status_poller = port_proxy_get_port_by_channel(port->port_proxy, CCCI_STATUS_RX);
		if (status_poller == NULL)
			CCCI_NORMAL_LOG(port->md_id, CHAR, "status_poller NULL, skip poller stop\n");
		else
...
READ_START:
	...
	if (skb == NULL) {
		CCCI_ERROR_LOG(port->md_id, CHAR, "%s:skb queue not empty but seed returen NULL %s\n",
					__func__, port->name);
		ret = -EFAULT;
		goto exit;
	}
