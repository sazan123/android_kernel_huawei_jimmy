...
char camera_dsm_log_buff[MTK_CAMERA_DSM_BUFFER_SIZE] = {0};
...
static char copy_buf[MTK_CAMERA_DSM_BUFFER_SIZE];
ssize_t camera_dsm_record_basic_info(struct dsm_client *pcamera_client , int type, int err_num , const char* str)
{
    ...
    /* Camera basic info */
    memset(copy_buf, 0, MTK_CAMERA_DSM_BUFFER_SIZE);
    len = snprintf(copy_buf, MTK_CAMERA_DSM_BUFFER_SIZE,
