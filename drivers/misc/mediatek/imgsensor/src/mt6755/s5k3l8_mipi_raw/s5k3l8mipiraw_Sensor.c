...
#define LOG_INF(fmt, args...)   if (DEBUG_S5K3L8) pr_debug(PFX "[%s] " fmt, __FUNCTION__, ##args)
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .hs_video = {
        ...
        .startx = 196,                    //record different mode's startx of grabwindow
        .starty = 16,                    //record different mode's starty of grabwindow
        .grabwindow_width  = 1000, //640,        //record different mode's width of grabwindow
        .grabwindow_height = 750, //480,        //record different mode's height of grabwindow
        /*     following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario    */
        .mipi_data_lp2hs_settle_dc = 85,
        /*     following for GetDefaultFramerateByScenario()    */
        .max_framerate = 1200,
...
// { 4208, 3120,    184,  120, 3840, 2880,  640,  480,   0,    0,  640,  480,      0, 0,  640,  480}, //hight speed video
 { 4208, 3120,    16,  384, 4176, 2352, 1392,  784,   196,    16, 1000,  750,      0, 0, 1000,  750},// gaoqi video
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    //gain = 64 = 1x real gain.
    kal_uint16 reg_gain = 0; // kal_uint16 reg_gain;

    LOG_INF("set_gain %d \n", gain);
    if (gain < BASEGAIN || gain > 16 * BASEGAIN) {
        pr_err("S5K3L8 Error gain setting");
        if (gain < BASEGAIN)
            gain = BASEGAIN;
...
//    spin_unlock(&imgsensor_drv_lock);
    switch (image_mirror) {
        ...
        case IMAGE_HV_MIRROR:
            ...
            break;
        default:
            pr_err("S5K3L8 Error image_mirror setting\n");
    }
...
#endif
                pr_err("S5K3L8 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
                return ERROR_NONE;
            }
            pr_err("S5K3L8 Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_err("S5K3L8 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            pr_err("S5K3L8 Read sensor id fail, id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
        ...
        pr_debug("S5K3L8 Warning:=== current_fps %d fps is not support, so use cap1's setting\n",imgsensor.current_fps/10);
        imgsensor.pclk = imgsensor_info.cap1.pclk;
        imgsensor.line_length = imgsensor_info.cap1.linelength;
        imgsensor.frame_length = imgsensor_info.cap1.framelength;
        imgsensor.min_frame_length = imgsensor_info.cap1.framelength;
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CUSTOM5:
            ...
            break;
        default:
            pr_err("S5K3L8 Error ScenarioId setting");
            preview(image_window, sensor_config_data);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    switch (scenario_id) {
        ...
        default:  //coding with  preview scenario by default
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
            pr_err("S5K3L8 error scenario_id = %d, we use preview scenario \n", scenario_id);
            break;
    }
    return ERROR_NONE;
}
