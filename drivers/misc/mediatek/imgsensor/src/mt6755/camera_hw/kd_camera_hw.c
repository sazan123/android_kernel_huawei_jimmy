...
PowerCust PowerCustList = {
    {
		...
		{GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_High},	/* MAIN2_DVDD; */
#else
               ...
               char *mode_name)
{
    ...
    /* power ON */
    printk("%s(%d) On = %d, pinSetIdx = %d byyzm\n",__FUNCTION__,__LINE__,On,pinSetIdx);
    if (On) {
        ...
        for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
            if (currSensorName && (PowerOnList.PowerSeq[pwListIdx].SensorName != NULL)
                ...
                strcmp(PowerOnList.PowerSeq[pwListIdx].SensorName,
                       currSensorName))) {
                //PK_DBG("sensorIdx:%d\n", SensorIdx);
                printk("%s(%d) currSensorName = %s byyzm\n",__FUNCTION__,__LINE__,currSensorName);
