...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin ==
				...
				if (pwInfo.Voltage == Vol_1200) {
					...
					PK_DBG
					    ("[CAMERA SENSOR] Main2 camera VCAM_D power 1.2V to 1.22V\n");
				}
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (pwInfo.Voltage == Vol_1200) {
					/* pwInfo.Voltage = Vol_1220; */
					PK_DBG
					    ("[CAMERA SENSOR] Sub camera VCAM_D power 1.2V to 1.22V\n");
				}
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mtkcam_gpio_set
			    (pinSetIdx, pwInfo.PowerType,
			     PowerCustList.PowerCustInfo[CUST_AFVDD].Voltage)) {
		...
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set
			    (pinSetIdx, PDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON])) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		} else {
			if (mtkcam_gpio_set
			    (pinSetIdx, PDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF])) {
		...
		if (pwInfo.Voltage == Vol_High) {
			if (mtkcam_gpio_set
			    (pinSetIdx, RST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON])) {
				PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
			}
		} else {
			if (mtkcam_gpio_set
			    (pinSetIdx, RST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF])) {
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mtkcam_gpio_set
			    (pinSetIdx, AVDD, 1 - PowerCustList.PowerCustInfo[CUST_AVDD].Voltage)) {
		...
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin ==
				...
				if (TRUE != _hwPowerDown(MAIN2_DVDD)) {
					PK_ERR
					    ("[CAMERA SENSOR] Fail to disable main2 digital power\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(SUB_DVDD)) {
					PK_ERR
					    ("[CAMERA SENSOR] Fail to enable sub digital power\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set
			...
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(DVDD)) {
					PK_ERR
					    ("[CAMERA SENSOR] Fail to disable main digital power\n");
					return FALSE;
				}
			} else {
				if (mtkcam_gpio_set
