...
#define DEBUG_OV13855 0
#define PFX "OV13855_camera_sensor"
#define LOG_INF(fmt, args...)   if (DEBUG_OV13855) pr_err(PFX "[%s] " fmt, __FUNCTION__, ##args)
...
bool read_ov13855_eeprom( kal_uint16 addr, unsigned char* data, kal_uint32 size){
    addr = 0x791;//from the first valid PD data on
...
//    _read_ov13855_eeprom(0x801, header, 9);
    ...
    memset(OV13855_eeprom_data, 0xff, DATA_SIZE);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        camera_back_probe_ok=1;
                pr_err("OV13855 Read sensor id ok  i2c write id: 0x%x, sensor id: 0x%x  info.sensor_id=0x%x\n",
                imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
                return ERROR_NONE;
            }
            pr_err("OV13855 Read sensor id fail:0x%x, sensor_id: 0x%x   info.sensor_id=0x%x\n",
            imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
            retry--;
        } while(retry > 0);
        i++;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_err("OV13855 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            pr_err("OV13855 Read sensor id fail: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
        ...
        if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
            pr_debug("OV13855 Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",imgsensor.current_fps,imgsensor_info.cap1.max_framerate/10);
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
            ...
            break;
        default:
            pr_err("OV13855 Error ScenarioId setting");
            preview(image_window, sensor_config_data);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    switch (scenario_id) {
        ...
        default:  //coding with  preview scenario by default
            ...
            set_dummy();
            pr_err("OV13855 error scenario_id = %d, we use preview scenario \n", scenario_id);
            break;
    }
    return ERROR_NONE;
}
