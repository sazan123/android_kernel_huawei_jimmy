...
#define PFX "gc8034mipi_Sensor.c"
...
/*Don't enable if you needn't debug */
...
static void gc8034_gcore_read_otp_info(void)
{
    ...
    kal_uint8 lsc[99*4]={0};
    kal_uint16 lscr[99]={0};
    kal_uint16 lscg[99]={0};
    kal_uint16 lscb[99]={0};
    kal_uint8 lscaddr = 0;
    kal_uint16 idx = 0;
    kal_uint8 ddtempbuff[80*4]={0};
    ...
    case 0x01:
        ...
        {
            LOG_INF("GC8034_OTP_DD:index = %d, x = %x, y = %x, check sum = %x, type = %x \n ",\
    ...
    for(index=0;index<2;index++)
    {
      ...
      switch((flag_lsc<<(2 * index))&0x0c)
        ...
        case 0x04:
             ...
             write_cmos_sensor(0xf4,0x98);
             write_cmos_sensor(0xfe,0x00);
             write_cmos_sensor(0x78,0x9b);//row and col
             write_cmos_sensor(0x79,0x0d);//enable
               write_cmos_sensor(0x7a,LSC_NUM);//lsc_num
               write_cmos_sensor(0x7c,LSC_ADDR[index*2]);//otp_lsc_page[4:2]
                write_cmos_sensor(0x7d,LSC_ADDR[index*2+1]);//otp_lsc_base_addr[7:0]
             ...
             write_cmos_sensor(0x79,0x00);
               write_cmos_sensor(0xfe,0x01);
               write_cmos_sensor(0xa0,0x11);//[0]lsc en
               write_cmos_sensor(0xfe,0x00);
             // mdelay(5);
             break;
        case 0x08:
...
extern bool camera_front_probe_ok;
/*************************************************************************
* FUNCTION
*    get_imgsensor_id
*
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("Read sensor id ok, write i2c id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);

                front_camera_find_success = 3;
                camera_front_probe_ok = 1;
                return ERROR_NONE;
            }
            LOG_INF("Read sensor id fail, write i2c id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
