...
static long flashlight_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_IS_CHARGER_READY:
		{
			printk(KERN_ERR "flashlight [FLASH_IOC_IS_CHARGER_READY] ioctl cmd run\n");
			kdArg.arg = 1;
			if (copy_to_user
				((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				printk(KERN_ERR"[FLASH_IOC_IS_CHARGER_READY] ioctl copy to user failed ~\n");
				return -EFAULT;
			}
			i4RetValue = 0;
		}
	...
	case FLASH_IOC_SET_SCENARIO:
		{
			printk(KERN_ERR" flashlight [FLASH_IOC_SET_SCENARIO] ioctl cmd run\n");
			i4RetValue = 0;
		}
		break;
	default:
                         ...
                         loff_t *data)
{
	...
	if (sscanf(regBuf, "%x %x",  &flashInfo.flash_id, &flashInfo.mode) == 2) {
		...
		}else if(flashInfo.flash_id & 0x010){
			if(flashInfo.mode){
				FL_dim_duty(0);
			}else{
				FL_Disable();
