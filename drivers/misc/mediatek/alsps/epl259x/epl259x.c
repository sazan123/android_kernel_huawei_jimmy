...
#define DRIVER_VERSION          "3.0.3_lsrc"
/******************************************************************************
...
#define ESD_PATCH       1
#if ESD_PATCH
static int esd_polling_delay = 2000;
#endif
#define ALSPS_DBG       1

#define LUX_PER_COUNT    374  //ALS lux per count
/*ALS interrupt table*/
...
#endif
#if ESD_PATCH
    struct delayed_work  esd_thd_polling_work;
#endif
#if HS_ENABLE || PS_GES
...
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_2048, EPL_ALS_INTT_1024};
static int als_dynamic_intt_value[] = {2048, 1024}; //match als_dynamic_intt_intt table
static int als_dynamic_intt_gain[] = {EPL_GAIN_MID, EPL_GAIN_LOW};
static int als_dynamic_intt_high_thr[] = {60000, 60000};
static int als_dynamic_intt_low_thr[] = {200, 2000};
static int als_dynamic_intt_intt_num =  sizeof(als_dynamic_intt_value)/sizeof(int);
static int als_dynamic_offset = 5;
#endif /*ALS_DYN_INTT*/
#if ALS_LSRC
int offset_gain = 600; // 1/2
...
static char stk3x1x_prox_version[]="mn25733d_mtk_1.0";//"stk3x1x_mtk_1.0";
static char stk3x1x_prox_vendor_name[20]="mn25733d";//"stk3x1x";
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.wait = EPL_WAIT_75_MS;
    ...
    c_gain = 374; // 300/1000=0.3 /*Lux per count*/
...
int lsrc_raw_convert_to_adc(u32 ch0, u32 ch1)
{
    ...
    {
        als_sample[sample_count].ir = ch0;
        als_sample[sample_count].als_data = ch1;
        als_sample[sample_count].ratio = lsrc_ratio;
        als_sample[sample_count].ratio1 = als_offset;
        als_sample[sample_count].als_value = nor_raw;
    }
#endif
    return nor_raw;
}
...
long raw_convert_to_lux(u16 raw_data)
{
    ...
    if(dyn_intt_raw > 0xffff)
        dyn_intt_raw = 0xffff;
    else if(dyn_intt_raw < als_dynamic_offset)
        dyn_intt_raw = 0;
    else
        dyn_intt_raw -= als_dynamic_offset;
    ...
    APS_LOG("[%s]:raw_data=%d, als_lsrc_raw=%d, lux=%ld\r\n", __func__, raw_data, als_lsrc_raw, lux);
#else
...
static int epl_sensor_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
    ...
    if(1 == atomic_read(&als_sample_en))
    {
        als_sample[sample_count].als_level = lsrc_lux;
        sample_count++;
    ...
    case CMC_BIT_PRE_COUNT:
        ...
        als_lux = (als * epl_sensor.als.factory.lux_per_count)/1000;
#ifdef ONTIM_SAMPLE
        if(1 == atomic_read(&als_sample_en))
        {
            als_sample[sample_count].als_level = lsrc_lux;
        ...
        case CMC_BIT_TABLE:
            ...
            {
                if(als < als_level[current_tp][current_color_temp][idx]/*obj->hw->als_level[idx]*/)
                {
                    break;
                }
            }
...
int epl_sensor_read_als(struct i2c_client *client)
{
...
#ifdef ONTIM_ALS_CALI
    channel[0] = epl_sensor.als.data.channels[0];
    channel[1] = epl_sensor.als.data.channels[1];
    als_load_cali();
    ...
    epl_sensor.als.data.channels[0] = epl_sensor.als.data.channels[0] *  als_ch0_expect / als_cali.als_ch0;
    epl_sensor.als.data.channels[1] = epl_sensor.als.data.channels[1] *  als_ch1_expect / als_cali.als_ch1;
#endif
#ifdef ONTIM_ALS_CALI
    if (als_cali_enable==2)
    {
        als_do_audo_cali(epl_sensor.als.data.channels[0],epl_sensor.als.data.channels[1]);
    }
#endif
    APS_LOG("als: ~~~~ ALS ~~~~~ \n");
...
#endif
#if ESD_PATCH
            cancel_delayed_work(&epld->esd_thd_polling_work);
#endif
            //wake_unlock(&ps_lock);
        }
...
#if ESD_PATCH
void epl_sensor_restart_esd_polling(void)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    cancel_delayed_work(&obj->esd_thd_polling_work);
    schedule_delayed_work(&obj->esd_thd_polling_work, msecs_to_jiffies(2*esd_polling_delay));
}
void epl_sensor_esd_thd_polling_work(struct work_struct *work)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    u8 buf[2];
    mutex_lock(&sensor_mutex);
    epl_sensor_I2C_Read(obj->client,0x5, 2);
    buf[0] = gRawData.raw_bytes[0];
    buf[1] = gRawData.raw_bytes[1];
    mutex_unlock(&sensor_mutex);
    ...
    {
        APS_LOG("[%s]: ESD happened...... REG0x05=0x%x, REG0x06 = 0x%x \r\n", __func__, buf[0], buf[1]);
    ...
    schedule_delayed_work(&obj->esd_thd_polling_work, msecs_to_jiffies(esd_polling_delay));
}
#endif
#if PS_DYN_K
void epl_sensor_restart_dynk_polling(void)
...
#endif
    {
        epl_sensor_restart_esd_polling();
    }
#endif
#if (PS_GES && !PS_DYN_K)
    if(enable_ges == 1 && epl_sensor.ges.polling_mode == 1)
...
static ssize_t epl_sensor_show_status(struct device_driver *ddri, char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "lsrc_lux = %d, als_lux = %d \n", lsrc_lux, als_lux);
...
#if ALS_LSRC
static ssize_t epl_sensor_store_lsrc_offset(struct device_driver *ddri, const char *buf, size_t count)
{
    int offset=0;
...
#endif
#if ALS_LSRC
static DRIVER_ATTR(als_lsrc_offset,             S_IWUSR | S_IRUGO, NULL,                               epl_sensor_store_lsrc_offset                    );
#endif
#ifdef ONTIM_SAMPLE
...
#ifdef CONFIG_COMPAT
static long epl_sensor_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
        long err = 0;
        ...
        APS_ERR("epl_sensor_compat_ioctl cmd= %x\n", cmd);
...
/*----------------------------------for factory mode test---------------------------------------*/
        ...
        case ALSPS_SET_PS_THRESHOLD:
            ...
            epl_sensor.ps.high_threshold = threshold[0];
            epl_sensor.ps.low_threshold = threshold[1];
...
#ifdef CONFIG_COMPAT
    .compat_ioctl = epl_sensor_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
...
static void lsample_work(struct work_struct *work)
{
        ...
        if(1 == sscanf(sample_path, "%s", path))
        {
                ...
                if (IS_ERR(filep)) {
                        ...
                        {
                        sprintf(buf, "%d,%d,%d,%d,%d,%d\n",als_sample[i].ir,als_sample[i].als_data,als_sample[i].ratio,als_sample[i].ratio1,als_sample[i].als_value,als_sample[i].als_level);
...
static int als_get_data(int* value, int* status)
{
    int err = 0;
    u16 report_lux = 0;
    u32 lux_temp = 0; /* for calibrate by shengweiguang */
    struct epl_sensor_priv *obj = epl_sensor_obj;
    if(!obj)
    {
        APS_ERR("obj is null!!\n");
    ...
    lux_temp = report_lux;
    lux_temp = (lux_temp*98)/100;
    report_lux = (u16)lux_temp;
    *value = report_lux;
...
#endif
#if ESD_PATCH
    INIT_DELAYED_WORK(&obj->esd_thd_polling_work, epl_sensor_esd_thd_polling_work);
#endif
#if HS_ENABLE
    atomic_set(&obj->hs_suspend, 0);
