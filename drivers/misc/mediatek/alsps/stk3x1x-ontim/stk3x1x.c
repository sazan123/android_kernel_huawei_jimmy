...
struct stk3x1x_priv {
    ...
    atomic_t                firlength;
    struct ir_data_filter      ir_fir;
    atomic_t                ir_firlength;
#endif
    uint16_t ir_code;
...
static u32 stk_als_data[2];
static char stk3x1x_ir_als_data_name[20]="0,0";//"stk3x1x";
    DEV_ATTR_DECLARE(als_prox)
    DEV_ATTR_DEFINE("version",stk3x1x_prox_version)
    DEV_ATTR_DEFINE("vendor",stk3x1x_prox_vendor_name)
    DEV_ATTR_DEFINE("ir_als_data",stk3x1x_ir_als_data_name)
    DEV_ATTR_VAL_DEFINE("ir_data",stk_als_data,ONTIM_DEV_ARTTR_TYPE_VAL_32BIT)
    DEV_ATTR_VAL_DEFINE("als_data",&stk_als_data[1],ONTIM_DEV_ARTTR_TYPE_VAL_32BIT)
#ifdef ONTIM_CALI
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
    ...
    int ir_idx = 0;
    int ir_firlen = atomic_read(&obj->ir_firlength);
#endif
    do_gettimeofday(&tv);
        tv_android = tv;
    ...
    if(obj->ir_fir.num < ir_firlen)
    {
        obj->ir_fir.raw[obj->ir_fir.num] = ir_data;
        obj->ir_fir.sum += ir_data;
        obj->ir_fir.num++;
        obj->ir_fir.idx++;
    }
    else
    {
        ir_idx = obj->ir_fir.idx % ir_firlen;
        obj->ir_fir.sum -= obj->ir_fir.raw[ir_idx];
        obj->ir_fir.raw[ir_idx] = ir_data;
        obj->ir_fir.sum += ir_data;
        obj->ir_fir.idx++;
        ir_data = (obj->ir_fir.sum / ir_firlen);
    }
#endif
    stk_als_data[0] = (u32)ir_data;
    stk_als_data[1] = (u32)als_data;
    ...
    if(ir_data < als_comperator1)
            ...
            current_color_temp = D65_TEMP;//A_TEMP
    else
            current_color_temp = D65_TEMP;

#else
    //ontim_ratio = ir_data/als_data;
    if(ir_data < als_comperator1)
            current_color_temp = CWF_TEMP;
    else
    {
        if (als_data < 50)
        {
            if ( (ir_data*100/als_data) >  ontim_cal[0])
                current_color_temp = A_TEMP;
            else
                current_color_temp = D65_TEMP;
        }
        else if (als_data >= 2500)
        {
            if ( (ir_data*100/als_data) >  ontim_cal[49])
                current_color_temp = A_TEMP;
            else
                current_color_temp = D65_TEMP;
        }
        else
        {
            shang = als_data/50;
            yu = als_data%50;
            ontim_ratio = ontim_cal[shang-1] - (ontim_cal[shang-1] - ontim_cal[shang])*yu/50;
            if ( (ir_data*100/als_data) > ontim_ratio)
                current_color_temp = A_TEMP;
            else
                current_color_temp = D65_TEMP;
        }
    }
    sprintf(stk3x1x_ir_als_data_name, "%d,%d,%d,%d\n", ir_data, als_data, (ir_data*100/als_data), current_color_temp);
#endif
    ...
    *data = (u16)als_data_u32;
    ...
    if(1 == atomic_read(&als_sample_en))
    {
        ...
        als_sample[sample_count].als_data = als_data_sample;
...
static int stk3x1x_enable_als(struct i2c_client *client, int enable)
{
    ...
    else
    {
            ...
            memset(&obj->ir_fir, 0x00, sizeof(obj->ir_fir));
    }
#endif
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
    int ret;
        ...
        //u32 ints[2] = {0, 0};
        //struct platform_device *alspsPltFmDev;^M
    struct stk3x1x_priv *obj = i2c_get_clientdata(client);
...
static ssize_t stk3x1x_ps_Hthreshold_store(struct device_driver *ddri, const char *buf, size_t count)
{
    unsigned long value = 0;
    int ret;
    ...
    atomic_set(&stk3x1x_obj->ps_high_thd_val, value);

    if((ret = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val))))
    {
        pr_err(" stk3x1x PS set Hthreshold errro !!!\n");
    }

    return count;
}

static ssize_t stk3x1x_ps_Hthreshold_show(struct device_driver *ddri, char *buf)
{
    if(!stk3x1x_obj)
    {
        APS_ERR("stk3x1x_obj is null!!\n");
...
static ssize_t stk3x1x_ps_Lthreshold_store(struct device_driver *ddri, const char *buf, size_t count)
{
    unsigned long value = 0;
    int ret;
    ...
    atomic_set(&stk3x1x_obj->ps_low_thd_val, value);

    if((ret = stk3x1x_write_ps_low_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_low_thd_val))))
    {
        pr_err(" stk3x1x PS set Lthreshold errro !!!\n");
    }

    return count;
}

static ssize_t stk3x1x_ps_Lthreshold_show(struct device_driver *ddri, char *buf)
{
    if(!stk3x1x_obj)
    {
        APS_ERR("stk3x1x_obj is null!!\n");
...
static ssize_t stk3x1x_store_firlen(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    else if (value < 1)
    {
        ...
        atomic_set(&stk3x1x_obj->ir_firlength, 1);
        memset(&stk3x1x_obj->ir_fir, 0x00, sizeof(stk3x1x_obj->ir_fir));
    }
    else
    {
        atomic_set(&stk3x1x_obj->firlength, value);
        memset(&stk3x1x_obj->fir, 0x00, sizeof(stk3x1x_obj->fir));
        atomic_set(&stk3x1x_obj->ir_firlength, value);
        memset(&stk3x1x_obj->ir_fir, 0x00, sizeof(stk3x1x_obj->ir_fir));
    }
...
#endif
static DRIVER_ATTR(ps_high_threshold,    S_IWUSR | S_IRUGO, stk3x1x_ps_Hthreshold_show,  stk3x1x_ps_Hthreshold_store);
static DRIVER_ATTR(ps_low_threshold,    S_IWUSR | S_IRUGO, stk3x1x_ps_Lthreshold_show,  stk3x1x_ps_Lthreshold_store);
/*----------------------------------------------------------------------------*/
...
#endif
    &driver_attr_ps_high_threshold,
    &driver_attr_ps_low_threshold,
};
...
//-add by hzb for ontim debug
    ...
    atomic_set(&obj->ir_firlength, STK_FIR_LEN);
#endif
...
static int __init stk3x1x_init(void)
{
    ...
    const char *name = "mediatek,stk3x1x";
        struct alsps_hw *hw = get_alsps_dts_func(name, &stk3x13_hw);
#else
    struct alsps_hw *hw = get_cust_alsps_hw_stk3x1x();
#endif
