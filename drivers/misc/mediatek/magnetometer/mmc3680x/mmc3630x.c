...
static struct i2c_board_info __initdata mmc3630x_i2c_board_info = { I2C_BOARD_INFO(MMC3630X_DEV_NAME, 0x30) };
...
static int mmc3630x_open(struct inode *inode, struct file *file)
{
	...
	{
		MEMSIC_INFO("Open device node:mmc3630x\n");
	}
...
static int mmc3630x_release(struct inode *inode, struct file *file)
{
	...
	if(atomic_read(&obj->trace) & MEMSIC_CTR_DEBUG)
	{
		MEMSIC_INFO("Release device node:mmc3630x\n");
	}
	return 0;
}
...

{
	...
	switch (cmd)
	{
		...
		case MMC31XX_IOC_RM:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MEMSIC_ERR("MMC3630X_IOC_SET failed\n");
				return -EFAULT;
			}
		...
		case MMC31XX_IOC_RRM:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MEMSIC_ERR("MMC3630X_CTRL_REFILL failed\n");
				return -EFAULT;
			}
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MEMSIC_ERR("MMC3630X_CTRL_RESET failed\n");
				return -EFAULT;
			}
			/* wait external capacitor charging done for next SET/RESET */
			msleep(MMC3630X_DELAY_STDN);
		...
		case MMC31XX_IOC_READ:
			...
			if(I2C_RxData(data, 6) < 0)
			{
				MEMSIC_ERR("MMC3630X_IOC_READ failed\n");
				return -EFAULT;
			}
			vec[0] = data[1] << 8 | data[0];
			vec[1] = data[3] << 8 | data[2];
			...
			{
				MEMSIC_ERR("MMC3630X_IOC_READ: copy to user failed\n");
				return -EFAULT;
			}
			break;
		...
		case MMC31XX_IOC_READXYZ:
			...
			if(copy_to_user(argp, vec, sizeof(vec)))
			{
				MEMSIC_ERR("MMC3630X_IOC_READXYZ: copy to user failed\n");
				return -EFAULT;
			}
			break;
	...
	long ret = 0;
		...
		switch(cmd)
		{
			...
			case COMPAT_MMC3630X_IOC_READ_REG:
				...
				if(ret < 0)
				{
					MEMSIC_ERR("COMPAT_MMC3630X_IOC_READ_REG is failed!\n");
				}
				break;
			...
			case COMPAT_MMC3630X_IOC_WRITE_REG:
				...
				if(ret < 0)
				{
					MEMSIC_ERR("COMPAT_MMC3630X_IOC_WRITE_REG is failed!\n");
				}
			...
			case COMPAT_MMC3630X_IOC_READ_REGS:
				...
				if(ret < 0)
				{
					MEMSIC_ERR("COMPAT_MMC3630X_IOC_READ_REGS is failed!\n");
				}
				break;
...
//-add by hzb for ontim debug
	...
	if(err < 0)
	{
		MEMSIC_ERR("mmc3630x_device send TM cmd failed\n");
		goto exit_kfree;
	}
	...
	{
		MEMSIC_ERR("mmc3630x_device register failed\n");
...
static int __init mmc3630x_init(void)
{
	...
	i2c_register_board_info(hw->i2c_num, &mmc3630x_i2c_board_info, 1);
