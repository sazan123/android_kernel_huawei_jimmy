...
#define CAM_CALINF(format, args...)    printk(KERN_INFO PFX "[%s] " format, __func__, ##args)
#define CAM_CALDB(format, args...)     printk(KERN_DEBUG PFX "[%s] " format, __func__, ##args)
#define CAM_CALERR(format, args...)    printk(KERN_ERR PFX format, ##args)
#else
#define CAM_CALINF(x, ...)
#define CAM_CALDB(x, ...)
#define CAM_CALERR(format, args...)    printk(KERN_ERR PFX format, ##args)
...
#define COMM_DEVICE_ID         0xA0//0xA2
...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL] fail common_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
		return false;
	}
	/*CAM_CALDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr,
	*data,page,offset);*/
	CAM_CALDB("[CAM_CAL] common_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
	return true;
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		...
		if (byteread_cmos_sensor(i2c_id, curAddr, buff)) {
...
static long common_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
	stCAM_CAL_INFO_STRUCT __user *data;
	...
	CAM_CALDB("[CAMERA SENSOR] common_Ioctl_Compat,%p %p %x ioc size %d\n", filp->f_op ,
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		/**pu1Params = 0;*/
		i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, COMM_DEVICE_ID, ptempbuf->u4Length);
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv2);
		if (ktv2.tv_sec > ktv1.tv_sec)
			TimeIntervalUS = ktv1.tv_usec + 1000000 - ktv2.tv_usec;
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/*.ioctl = CAM_CAL_Ioctl*/
#ifdef CONFIG_COMPAT
	.compat_ioctl = common_Ioctl_Compat,
#endif
...
static struct i2c_driver CAM_CAL_i2c_driver = {
	...
	.driver.of_match_table = eeprom_of_match,
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = COMM_DEVICE_ID >> 1;
...
MODULE_DESCRIPTION("common CAM_CAL driver");
