...
#define CAM_CALINF(format, args...)    printk(KERN_ERR PFX "[%s] " format, __func__, ##args)
#define CAM_CALDB(format, args...)     printk(KERN_ERR PFX "[%s] " format, __func__, ##args)
...
#define s5k3l8_DEVICE_ID         0xA0//0xA2
...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALERR("[CAM_CAL] fail s5k3l8_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
		return false;
	}
	/*CAM_CALDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr,
	*data,page,offset);*/
	CAM_CALDB("[CAM_CAL] s5k3l8_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
	return true;
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		/*if(selective_read_byte(addr,(u8*)buff,s5k3l8_DEVICE_ID)){*/
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		/**pu1Params = 0;*/
		i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, s5k3l8_DEVICE_ID, ptempbuf->u4Length);
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv2);
		if (ktv2.tv_sec > ktv1.tv_sec)
			TimeIntervalUS = ktv1.tv_usec + 1000000 - ktv2.tv_usec;
