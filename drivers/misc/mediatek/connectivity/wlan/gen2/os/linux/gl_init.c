...
/* prGlueInfo->rHifInfo.DmaOps->DmaRegDump(&prGlueInfo->rHifInfo); */
                ...
                wifi_dsm_report_num(DSM_WIFI_FAIL_LOADFIRMWARE_ERR,"firmware load failed",i4Status);
#endif
				break;
			}
		}
...
#ifdef CONFIG_HUAWEI_DSM
            wifi_dsm_report_num(DSM_WIFI_DRIVER_LOAD_ERR,"driver load failure",i4Status);
#endif
			break;
		}
...
static int initWlan(void)
{
	...
	if (ret == -EIO) {
		kalUninitIOBuffer();
#ifdef CONFIG_HUAWEI_DSM
        wifi_dsm_report_num(DSM_WIFI_MODULE_INIT_ERROR, "module init error", ret);
#endif
		return ret;
	}
