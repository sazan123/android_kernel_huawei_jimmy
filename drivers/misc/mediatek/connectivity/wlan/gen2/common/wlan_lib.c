...
#ifdef CONFIG_HUAWEI_DSM
		wifi_dsm_report_num(DSM_WIFI_SDIO_PROBE_ATTACH_ERROR, "probe attach error", u4Status);
#endif
...
WLAN_STATUS wlanProcessCommandQueue(IN P_ADAPTER_T prAdapter, IN P_QUE_T prCmdQue)
{
	...
	while (prQueueEntry) {
		...
		default:
			ASSERT(0);
#ifdef CONFIG_HUAWEI_DSM
            wifi_dsm_report_num(DSM_WIFI_DHD_DEV_INIT_IOCTL_ERROR, "bad cmd", prCmdInfo->eCmdType);
#endif
			break;
		}
...
int wifi_dsm_register(void)
{
	...
	{
		DBGLOG(INIT, ERROR, "wifi_dclient had been register!\n");
		return 0;
	}
	wifi_dclient = dsm_register_client(&wifi_dsm_info);
	if (NULL == wifi_dclient)
	{
		DBGLOG(INIT, ERROR, "wifi_dclient register failed!\n");
	}
	return 0;
}
...
int wifi_dsm_report_num(int dsm_err_no, char *err_msg, int err_code)
{
	...
	if (NULL == wifi_dclient)
	{
		DBGLOG(INIT, ERROR, "wifi_dclient did not register!\n");
		return 0;
	}
	err = dsm_client_ocuppy(wifi_dclient);
	if (0 != err)
	{
		DBGLOG(INIT, ERROR, "user buffer is busy!\n");
		return 0;
	}
...
int wifi_dsm_report_info(int error_no, void *log, int size)
{
	...
	if ((error_no < DSM_WIFI_ERR) || (error_no > DSM_WIFI_ROOT_NOT_RIGHT_ERR)
		|| (NULL == log) || (size < 0))
	{
		DBGLOG(INIT, ERROR, "input param error!\n");
		return 0;
	}
...
void wifi_dsm_release_client(void)
{
	if (wifi_dclient)
	{
