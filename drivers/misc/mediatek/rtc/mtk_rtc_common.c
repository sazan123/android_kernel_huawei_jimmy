...
void rtc_mark_check_prj_err(void)
{
	unsigned long flags;

	rtc_xinfo("rtc_mark_check_prj_err\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_PRJ_ERR, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}
//-add by hzb for check cust prj ver

void rtc_mark_check_binding(void)
{
	unsigned long flags;

	rtc_xinfo("rtc_mark_check_binding\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_BINDING, 0x1);
	spin_unlock_irqrestore(&rtc_lock, flags);
}
