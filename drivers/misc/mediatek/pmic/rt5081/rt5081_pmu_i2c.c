...
static int rt5081_pmu_read_device(void *i2c, u32 addr, int len, void *dst)
{
	...
	struct rt5081_pmu_chip *chip = i2c_get_clientdata(client);
...
#ifdef CONFIG_MTK_I2C_EXTENSION
	i2c->timing = 400; /* 3.4MHz */
	chip->rt_DMABuf_va = dmam_alloc_coherent(&i2c->dev, 256, &chip->rt_DMABuf_pa,
						 GFP_KERNEL);
	if (!chip->rt_DMABuf_va) {
		dev_err(&i2c->dev, "dma alloc error\n");
		goto out_regmap;
	}
#endif /* #ifdef CONFIG_MTK_I2C_EXTENSION */
