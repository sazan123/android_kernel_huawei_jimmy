...
struct dsm_client *sensorhub_dclient = NULL;
#endif
...
#if defined (CONFIG_HUAWEI_DSM)
        if (!dsm_client_ocuppy(sensorhub_dclient)) {
            ACC_LOG("try to client record 926007001 \n");
            dsm_client_record(sensorhub_dclient, "Gsensor data abnormal\n");
            dsm_client_notify(sensorhub_dclient, DSM_GS_DATA_ERROR);
}
#endif
		goto acc_loop;
	} else {
...
#if defined (CONFIG_HUAWEI_DSM)
        if (!dsm_client_ocuppy(sensorhub_dclient)) {
            ACC_LOG("try to client record 926007003 \n");
            dsm_client_record(sensorhub_dclient, "Gsensor data not update\n");
            dsm_client_notify(sensorhub_dclient, DSM_GS_DATA_TIMES_NOTCHANGE_ERROR);
}
#endif
			goto acc_loop;
...
#if defined (CONFIG_HUAWEI_DSM)
    if (!sensorhub_dclient) {
        sensorhub_dclient = dsm_register_client(&dsm_sensorhub);
}
#endif
...
static int acc_remove(void)
{
	...
	kfree(acc_context_obj);
#if defined (CONFIG_HUAWEI_DSM)
    dsm_unregister_client(sensorhub_dclient, &dsm_sensorhub);
#endif
	return 0;
}
