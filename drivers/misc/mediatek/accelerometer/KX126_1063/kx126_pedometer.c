...
static int kx126_sc_init_client(bool enable)
{
	...
	if (0 != err) {
		STEP_C_ERR("KX126_1063_init_pedometer fail\n");
		return 1;
	}
	return 0;
}
...
static int kx126_sc_enable_nodata(int en)
{
	...
	if (err) {
		STEP_C_ERR("KX126_1063_ReadData error: %d\n", err);
		return err;	
	}
...
static int kx126_sc_get_data_step_c(uint32_t *value, int *status)
{
    ...
    if( (KX126_1063_suspend_flag)){ 
        STEP_C_ERR("kx126_sc_get_data_step_c: gsensor resume not ok\n");
...
static int kx126_sc_attr_create(void)
{
	...
	if (res) {
		STEP_C_ERR("kx126_a_device misc register failed!\n");
	...
	if (res) {
		STEP_C_ERR("kx126_sc create attribute err = %d\n", res);
...
static int kx126_sc_attr_remove(void)
{
	...
	if (res)
		STEP_C_ERR("kx126_sc_delete_attr fail: %d\n", res);
	else
	  	res = 0;

	res = misc_deregister(&kx126_sc_device);
	if (res)
		STEP_C_ERR("KX126 misc_deregister fail: %d\n", res);
	else
		res = 0;
