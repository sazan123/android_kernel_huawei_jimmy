...
int __batt_meter_init_cust_data_from_cust_header(struct platform_device *dev)
{
    ...
    batt_meter_cust_data.r_charger_1 = g_R_CHARGER_1 = R_CHARGER_1_MODIFIED;
    hw_version = get_hw_ver_info();
    if (hw_version == NULL)
    {
        pr_err("get hw version error! please check the boardID voltage!\n");
        return -1;
    }
    else
        global_hw_type = hw_version->hw_type;
    if (global_hw_type == 1)//modified vcdt
    {
        batt_meter_cust_data.r_charger_1 = g_R_CHARGER_1 = R_CHARGER_1_MODIFIED;
        pmic_config_interface(0xF8E, 0x02, 0xF, 0);
    }
    else
    {
        batt_meter_cust_data.r_charger_1 = g_R_CHARGER_1 = R_CHARGER_1;
        pmic_config_interface(0xF8E, 0x0, 0xF, 0);
    }
    pr_err("global_hw_type = %d\n", global_hw_type);
	return 0;
}
