...
{
       {0, 3900, 4280, 0x02}, {3900, 4000, 4440, 0x03},
       {4000, 4250, 4520, 0x04}, {4250, 4590, 4600, 0x05}
};
static vbat_area vbatAreaForOld[] =
{
       {0, 3900, 4280, 0x05}, {3900, 4000, 4440, 0x05},
       {4000, 4250, 4520, 0x05}, {4250, 4590, 4600, 0x05}
};
...
void __set_dpm(int indx, int *last_indx, u8 *tmpdpm)
{
       global_dpm = vbatArea[indx].dpm;
       *last_indx = indx;
       if (global_hw_type == 0)
            *tmpdpm = vbatAreaForOld[indx].dpm_byte;
       else
            *tmpdpm = vbatArea[indx].dpm_byte;
       battery_charging_control(CHARGING_CMD_SET_DPM, tmpdpm);
       return;
}
void set_dpm_proc(unsigned int vbat,u32 charger_enable)
{
   int indx;
   u8 tmpdpm;
   static unsigned int last_indx = -1;
   if(KAL_FALSE == charger_enable)
   {
       global_dpm = 0;
       battery_charging_control(CHARGING_CMD_SET_DPM, &vbatArea[0].dpm_byte);
       return ;
   }
   ...
   {
       __set_dpm(indx, &last_indx, &tmpdpm);
       return;
   }
   if (vbatArea[indx].dpm < global_dpm)
   {
       if (vbatArea[indx].max - vbat >= 50)
       {
           __set_dpm(indx, &last_indx, &tmpdpm);
           return;
       }
...
static unsigned int charging_full_check(void)
{
	...
	if (status == KAL_TRUE) {
		...
		if(BMT_status.charger_exist == KAL_TRUE)
		{
            ...
            if (bat_vol < 4300 && (BMT_status.temperature > 45 || BMT_status.temperature < 0))
            {
...
static void pchr_turn_on_charging(void)
{
   ...
   bat_vol = battery_meter_get_battery_voltage(KAL_TRUE);
   set_dpm_proc(bat_vol, charging_enable);
