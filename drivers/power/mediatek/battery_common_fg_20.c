...
void mt_battery_GetBatteryData(void)
{
	...
	if (batteryIndex >= BATTERY_AVERAGE_SIZE)
		batteryIndex = 0;
	battery_log(BAT_LOG_CRTI,
		"[kernel]AvgVbat %d,bat_vol %d, AvgI %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, CHR_Type %d, SOC %3d:%3d:%3d, bcct %d:%d, Ichg %d, IBat %d, car_tune_value %d global_hw_type %d\n",
		...
		BMT_status.IBattery / 10, batt_meter_cust_data.car_tune_value, global_hw_type);
}
...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
            ...
            set_dpm_proc(BMT_status.bat_vol, KAL_FALSE);
...
int __batt_init_cust_data_from_cust_header(void)
{
    ...
    if (global_hw_type == 1)
	    batt_cust_data.v_charger_max = V_CHARGER_MAX_MODIFIED;
    else
