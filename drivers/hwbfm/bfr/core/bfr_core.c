    ...
    char *args)
{
    ...
    bfr_recovery_record_t *cur_recovery_record = NULL;
    printk(KERN_ERR "func: %s line: %d",__func__, __LINE__);
	...
	cur_recovery_record = kmalloc(sizeof(bfr_recovery_record_t), GFP_KERNEL);
	if (cur_recovery_record == NULL)
	{
		return FRM_DO_NOTHING;
	}
    memset((void *)cur_recovery_record, 0, sizeof(bfr_recovery_record_t));
    ...
    if (0 != ret)
    {
        ...
        kfree(cur_recovery_record);
		cur_recovery_record = NULL;
        return FRM_DO_NOTHING;
    }
