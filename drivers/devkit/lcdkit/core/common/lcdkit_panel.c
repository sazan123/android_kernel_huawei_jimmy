...
static ssize_t lcdkit_check_reg_show(void* pdata, char* buf)
{
	int ret = 0;
	int i = 0;
	char expect_ptr[3] = {0x83,0x94,0xf};
	unsigned int count;
...
for (count = 0; count < LCDKIT_REG_CHECK_NUM; count++)
	{
		ret = 0;
		lcdkit_dsi_rx(0xda,&value1);//83
		lcdkit_dsi_rx(0xdb,&value2);//94
		lcdkit_dsi_rx(0xdc,&value3);//f

		if((value1 != expect_ptr[0]) && (value1 != expect_ptr[1]) && (value2 != expect_ptr[2]))
		{
			ret = 1;
			LCDKIT_INFO("%s value1=0x%0x,value2=0x%0x,value3=0x%0x\n",__func__,value1,value2,value3);
			LCDKIT_INFO("%s expect_ptr[0]=0x%0x,expect_ptr[1]=0x%0x,expect_ptr=0x%0x\n",__func__,expect_ptr[0],expect_ptr[1],expect_ptr[2]);
			break;
		}
		LCDKIT_INFO("%s value1=0x%0x,value2=0x%0x,value3=0x%0x\n",__func__,value1,value2,value3);
		LCDKIT_INFO("%s expect_ptr[0]=0x%0x,expect_ptr[1]=0x%0x,expect_ptr=0x%0x\n",__func__,expect_ptr[0],expect_ptr[1],expect_ptr[2]);
