...
static void validate_diff_self_rx_result(int diff_threshold)
{
	struct parade_core_data *cd = NULL;
	struct cmcp_data *cmcp_info = NULL;
	struct result *result = NULL;
	int32_t rx_num = CY_RAW_RX_NUM;
	int i = 0;
	...
	for(i = 0; i < rx_num; i++)
	{
		if(ABS(cmcp_info->diff_self_data[i]) > diff_threshold)
		{
			result->diff_self_rx_test_pass= false;
			TS_LOG_ERR("%s: self noise data-%d=%d breached threshold\n", __func__,
...
static int parade_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if (CY_SELF_NOISE_SUP == cd->self_noise_supported) {
		validate_diff_self_rx_result(configuration->diff_self_rx_noise_threshold);
	}
	...
	if (CY_SELF_NOISE_SUP == cd->self_noise_supported) {
		if(result->diff_self_rx_test_pass){
