...
static int32_t nvt_tddi_read_fw_noise(int32_t *xdata)
{
	...
	char file_path[64] = {"\0"};
	loff_t pos = NO_ERR;
	int32_t write_ret = NO_ERR;
	uint32_t output_len = NO_ERR;
	...
	fbufp = (char *)vmalloc(RAW_DATA_SIZE);
	if (!fbufp) {
		TS_LOG_ERR("%s: vmalloc for fbufp failed!\n", __func__);
		return -ENOMEM;
	}
	...
	for (y = NO_ERR; y < Y_Channel; y++) {
		for (x = NO_ERR; x < X_Channel; x++) {
			iArrayIndex = y * X_Channel + x;
			printk("%5d ", RawData_Diff[iArrayIndex]);
			snprintf(fbufp + iArrayIndex * NVTTDDI_MULTIPLY_7_NUM  + y * NVTTDDI_MULTIPLY_2_NUM, sizeof(fbufp),"%5d, ", RawData_Diff[iArrayIndex]) ;
		}
		printk("\n");
		snprintf(fbufp + (iArrayIndex + NVTTDDI_PLUS_ONE) * NVTTDDI_MULTIPLY_7_NUM + y * NVTTDDI_MULTIPLY_2_NUM,sizeof(fbufp),"\r\n");
	}
	printk("\n");
	...
	fp = filp_open(file_path, O_RDWR | O_CREAT, 0644);
	if (fp == NULL || IS_ERR(fp)) {
		TS_LOG_ERR("%s: filp_open %s failed\n", __func__, file_path);
	} else {
		output_len = (Y_Channel * X_Channel * NVTTDDI_MULTIPLY_7_NUM + Y_Channel * NVTTDDI_MULTIPLY_2_NUM);
		write_ret = vfs_write(fp, (char __user *)fbufp, output_len, &pos);
		if (write_ret <= NO_ERR) {
			TS_LOG_ERR("%s: vfs_write %s failed\n", __func__, file_path);
		}
	}
	set_fs(org_fs);
	if (fp) {
...
static int32_t nvt_tddi_read_fw_open(void)
{
	...
	for (y = NO_ERR; y < IC_Y_CFG_SIZE; y++) {
		for (x = NO_ERR; x < IC_X_CFG_SIZE; x++) {
			printk("%5d, ", RawData_Open[y * IC_X_CFG_SIZE + x]);
			snprintf(fbufp + NVTTDDI_MULTIPLY_7_NUM * (y * IC_X_CFG_SIZE + x) + y * NVTTDDI_MULTIPLY_2_NUM, sizeof(fbufp),"%5d, ", RawData_Open[y * IC_X_CFG_SIZE + x]) ;
		}
		printk("\n");
		snprintf(fbufp + NVTTDDI_MULTIPLY_7_NUM * (y * IC_X_CFG_SIZE + x) + y * NVTTDDI_MULTIPLY_2_NUM,sizeof(fbufp),"\r\n");
	}
	printk("\n");
	...
	if (fp == NULL || IS_ERR(fp)) {
		...
		output_len = IC_Y_CFG_SIZE * IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_7_NUM + IC_Y_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM;
		write_ret = vfs_write(fp, (char __user *)fbufp, output_len, &pos);
		if (write_ret <= NO_ERR) {
			TS_LOG_ERR("%s: vfs_write %s failed\n", __func__, file_path);
		}
...
int32_t nvt_kit_selftest(struct ts_rawdata_info *info)
{
	...
	if (open_ret != NO_ERR) {
		...
		//---Self Test Check --- // 0:PASS, -1:FAIL
		if (nvt_ts->open_test_by_fw > NO_ERR) {
			TestResult_Open = nvt_rawdata_up_low(RawData_Open, RecordResult_Open, IC_X_CFG_SIZE, IC_Y_CFG_SIZE,
	...
	if(nvt_ts->criteria_threshold_flag){
		...
		info->buff[0] = IC_X_CFG_SIZE;
		info->buff[1] = IC_Y_CFG_SIZE;
		info->used_size = IC_X_CFG_SIZE * IC_Y_CFG_SIZE * 4  + CHANNEL_NUM;//4:rawdata+diff+short+open

		if(info->used_size < TS_RAWDATA_BUFF_MAX){//buff is enough for rawdata, diff, short and open

			bytes_of_array = IC_X_CFG_SIZE*IC_Y_CFG_SIZE*sizeof(int);
			memcpy(&info->buff[CHANNEL_NUM], RawData_FWMutual, bytes_of_array);

			uint32_t start_p = IC_X_CFG_SIZE * IC_Y_CFG_SIZE + CHANNEL_NUM;
			memcpy(&info->buff[start_p], RawData_Diff, bytes_of_array);

			start_p = IC_X_CFG_SIZE * IC_Y_CFG_SIZE*2 + CHANNEL_NUM;
			memcpy(&info->buff[start_p],RawData_Short,bytes_of_array);

			start_p = IC_X_CFG_SIZE * IC_Y_CFG_SIZE*3 + CHANNEL_NUM;
		...
		info->used_size = X_Channel * Y_Channel * 2  + CHANNEL_NUM;//2:rawdata + diff
		bytes_of_array = (X_Channel*(uint64_t)Y_Channel*sizeof(int));
		memcpy(&info->buff[CHANNEL_NUM], RawData_FWMutual, bytes_of_array);
		memcpy(&info->buff[X_Channel *(uint64_t) Y_Channel + CHANNEL_NUM], RawData_Diff, bytes_of_array);
	}
