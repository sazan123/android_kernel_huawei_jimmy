	...
	if (f54->rmi4_data->synaptics_chip_data->raw_test_type == RAW_DATA_TEST_TYPE_SINGLE_POINT) {
		...
		for (i = 0; i < mmi_buf_size; i += 2) {
			...
			if (synaptics_raw_data_limit_flag) {
				raw_cap_uplimit = Initraw_cap_uplimit;
				raw_cap_lowlimit = Initraw_cap_lowlimit;
			}
...
static int f54_delta_rx_report(void)
{
	...
	for (i = 0; i < mmi_buf_size; i += 2) {
		...
		if (synaptics_raw_data_limit_flag) {
			delt_cap_uplimit = Initdelt_cap_uplimit;
			delt_cap_lowlimit = Initdelt_cap_lowlimit;
		}
...
static int f54_delta_tx_report(void)
{
	...
	for (tx_n = 0; tx_n < f54->rmi4_data->num_of_tx - 1; tx_n++) {
		for (rx_n = 0; rx_n < f54->rmi4_data->num_of_rx; rx_n++) {
			if (synaptics_raw_data_limit_flag) {
				tx_to_tx_cap_uplimit =Inittx_to_tx_cap_uplimit;
				tx_to_tx_cap_lowlimit =Inittx_to_tx_cap_lowlimit;
			}
			Rawimage_delta_tx =
...
static int match_module_name(const char *module_name)
{
	...
	if (strcmp(rmi4_data->synaptics_chip_data->ts_platform_data->
			product_name, "jimmy") == 0) {
		if (strcmp(module_name, "ofilm") == 0) {
			RawCapUpperLimit = RawCapUpperLimit_ofilm_jimmy;
			RawCapLowerLimit = RawCapLowerLimit_ofilm_jimmy;
			RxDeltaCapUpperLimit = RxDeltaCapUpperLimit_ofilm_jimmy;
			RxDeltaCapLowerLimit = RxDeltaCapLowerLimit_ofilm_jimmy;
			TxDeltaCapUpperLimit = TxDeltaCapUpperLimit_ofilm_jimmy;
			TxDeltaCapLowerLimit = TxDeltaCapLowerLimit_ofilm_jimmy;
			return NO_ERR;
		} else {
