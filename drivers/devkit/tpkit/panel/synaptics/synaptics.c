...
int synaptics_schedule_normal(void);
int synaptics_schedule_gesture(void);
DECLARE_WORK(synaptics_resume_normal_work,synaptics_schedule_normal);
DECLARE_WORK(synaptics_resume_gesture_work,synaptics_schedule_gesture);
...
static int synaptics_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (unlikely
	    ((atomic_read(&rmi4_data->synaptics_chip_data->ts_platform_data->state) == TS_SLEEP)
         ...
         || (atomic_read(&rmi4_data->synaptics_chip_data->ts_platform_data->state) == TS_RESUMING))) {
...
static void synaptics_put_device_into_easy_wakeup(void)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
					 rmi4_data->rmi4_feature.
					 f01_ctrl_base_addr, &sleep_ctrl,
					 sizeof(sleep_ctrl));
	if (retval < 0) {
		TS_LOG_ERR("Failed to read sleep mode\n");
		return;
	}
	sleep_ctrl = (sleep_ctrl & ~NO_SLEEP_ON);
...
static void synaptics_release_all_finger(void)
{
    ...
    if(rmi4_data->synaptics_chip_data->ts_platform_data->input_dev) {
        input_mt_sync (rmi4_data->synaptics_chip_data->ts_platform_data->input_dev);
        input_report_key(rmi4_data->synaptics_chip_data->ts_platform_data->input_dev, BTN_TOUCH, 0);
        input_sync(rmi4_data->synaptics_chip_data->ts_platform_data->input_dev);
        TS_LOG_DEBUG("%s: input_dev->name = %s\n", __func__,rmi4_data->synaptics_chip_data->ts_platform_data->input_dev->name);
    }
    TS_LOG_INFO("%s: end\n", __func__);
}
...
static int synaptics_resume(void)
{
	...
	case TS_POWER_OFF_MODE:
        ...
        #else
		schedule_work(&synaptics_resume_normal_work);
        #endif
		break;
	case TS_GESTURE_MODE:
        #if 0
		...
		synaptics_hardware_reset_chip();
        #else
		schedule_work(&synaptics_resume_gesture_work);
        #endif
		break;
	default:
