...
ssize_t hmx_dsm_record_freeze_err_info(int err_numb)
{
	ssize_t size = 0;
	ssize_t total_size = 0;
	TS_LOG_INFO("%s: entry!\n", __func__);
	/* err number */
	size = dsm_client_record(hmx_tp_dclient, "TP  find freeze:%d\n", err_numb );
	total_size += size;
...
int hmx_tp_report_dsm_err(int type, int err_numb)
{
	...
	switch(type)
	{
		...
		case DSM_TP_ESD_ERROR_NO:
			...
			break;
		case DSM_TP_FREEZE_ERROR_NO:
			/* report tp basic infomation */
			hmx_dsm_record_basic_err_info();
			/* report esd infomation */
			hmx_dsm_record_freeze_err_info(err_numb);
			break;
		default:
			break;
	}
