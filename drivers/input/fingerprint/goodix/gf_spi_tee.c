...
static char version[]="gf3258/3268_mtk_1.0";
static char vendor_name[20]="Truly3258/KaiEr3268";
...
#define GF_INPUT_NAME "fingerprint"
...
static DEVICE_ATTR(debug, S_IRUGO | S_IWUSR, gf_debug_show,NULL);
...
const struct mt_chip_conf spi_ctrdata_goodix = {
    .setuptime = 10,
    .holdtime = 10,
    .high_time = 50, /* 1MHz */
    .low_time = 50,
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
...
#if 0
    gf_dev->pins_irq = pinctrl_lookup_state(gf_dev->pinctrl, "fingerprint_irq");
    //printk("ly-test -----> gf_dev->pins_irq = %d \n",(int)gf_dev->pins_irq);
    ...
    gf_dev->rst_output1 = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_reset_en1");
    if (IS_ERR(gf_dev->rst_output1)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_reset_en1\n");
        return PTR_ERR(gf_dev->rst_output1);
    }

    gf_dev->rst_output0 = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_reset_en0");
    if (IS_ERR(gf_dev->rst_output0)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_reset_en0\n");
        return PTR_ERR(gf_dev->rst_output0);
    }

    gf_dev->cs_finger_spi0_mi_as_spi0_mi = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_mi_as_spi0_mi");
    if (IS_ERR(gf_dev->cs_finger_spi0_mi_as_spi0_mi)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_mi_as_spi0_mi\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_mi_as_spi0_mi);
    }

    gf_dev->cs_finger_spi0_mi_as_gpio = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_mi_as_gpio");
    if (IS_ERR(gf_dev->cs_finger_spi0_mi_as_gpio)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_mi_as_gpio\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_mi_as_gpio);
    }

    gf_dev->cs_finger_spi0_mo_as_spi0_mo = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_mo_as_spi0_mo");
    if (IS_ERR(gf_dev->cs_finger_spi0_mo_as_spi0_mo)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_mo_as_spi0_mo\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_mo_as_spi0_mo);
    }

    gf_dev->cs_finger_spi0_mo_as_gpio = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_mo_as_gpio");
    if (IS_ERR(gf_dev->cs_finger_spi0_mo_as_gpio)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_mo_as_gpio\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_mo_as_gpio);
    }

    gf_dev->cs_finger_spi0_clk_as_spi0_clk = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_clk_as_spi0_clk");
    if (IS_ERR(gf_dev->cs_finger_spi0_clk_as_spi0_clk)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_clk_as_spi0_clk\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_clk_as_spi0_clk);
    }

    gf_dev->cs_finger_spi0_clk_as_gpio = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_clk_as_gpio");
    if (IS_ERR(gf_dev->cs_finger_spi0_clk_as_gpio)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_clk_as_gpio\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_clk_as_gpio);
    }

    gf_dev->cs_finger_spi0_cs_as_spi0_cs = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_cs_as_spi0_cs");
    if (IS_ERR(gf_dev->cs_finger_spi0_cs_as_spi0_cs)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_cs_as_spi0_cs\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_cs_as_spi0_cs);
    }

    gf_dev->cs_finger_spi0_cs_as_gpio = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_spi0_cs_as_gpio");
    if (IS_ERR(gf_dev->cs_finger_spi0_cs_as_gpio)) {
        gf_debug(DEBUG_LOG,"Cannot find cs_finger_spi0_cs_as_gpio\n");
        return PTR_ERR(gf_dev->cs_finger_spi0_cs_as_gpio);
    }

    gf_dev->eint_as_int = pinctrl_lookup_state(gf_dev->pinctrl, "cs_finger_int_as_int");
    if (IS_ERR(gf_dev->eint_as_int)) {
        gf_debug(DEBUG_LOG,"Cannot find s_finger_int_as_int\n");
        return PTR_ERR(gf_dev->eint_as_int);
    }
...
//      hwPowerOn(MT6331_POWER_LDO_VIBR, VOL_2800, "fingerprint");
        enable = 0;
        #ifdef CONFIG_OF
        pinctrl_select_state(gf_dev->pinctrl, gf_dev->rst_output0);
        mdelay(15);
        pinctrl_select_state(gf_dev->pinctrl, gf_dev->rst_output1);
        #endif
    }
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
    ...
    pinctrl_select_state(gf_dev->pinctrl, gf_dev->eint_as_int);

    node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
    if (node) {
        gf_dev->irq_num = irq_of_parse_and_map(node, 0);
        gf_debug(INFO_LOG,"ly-test ---> %s, gf_irq = %d\n", __func__, gf_dev->irq_num);
...
//#ifdef CONFIG_OF
//      if (flag == 0) {
//          pinctrl_select_state(gf_dev->pinctrl, gf_dev->pins_miso_pulllow);
//          gf_debug(INFO_LOG, "%s: set miso PIN to low\n", __func__);
//          flag = 1;
//      } else {
//          pinctrl_select_state(gf_dev->pinctrl, gf_dev->pins_miso_pullhigh);
...
static void gf_miso_gpio_cfg(struct gf_device *gf_dev, u8 pullhigh)
{
    ...
    if (pullhigh)
        ...
        pinctrl_select_state(gf_dev->pinctrl, gf_dev->pins_miso_spi);
...
static int gf_probe(struct spi_device *spi)
{
    ...
    memcpy(&gf_dev->spi_mcc, &spi_ctrdata_goodix, sizeof(struct mt_chip_conf));
...
/*
    {
        ...
        u8 buffer_1[10]={0};
        u8 buffer_2[10]={0};
        printk("hqd add read chipid");
        gf_spi_read_bytes_ree(gf_dev,0x0000,4,buffer_1);
        for(i=0;i<10;i++)
        {
            printk("hqd add buffer_1[%d] =0x%x\n",i,buffer_1[i]);
        }
        gf_spi_read_bytes_ree(gf_dev,0x0002,4,buffer_2);
        for(i=0;i<10;i++)
        {
            printk("hqd add buffer_2[%d] =0x%x\n",i,buffer_2[i]);
        }
    }
*/
    /* create class */
...
static int __init gf_init(void)
{
    ...
    FUNC_ENTRY();
    if (get_fp_vendor() !=2 ){
        printk("[goodix]not find goodix fingerprint sensor, get_fp_vendor=%d!\n",get_fp_vendor());
        return -1;
    }

    status = spi_register_board_info(chips_spi_board_info,ARRAY_SIZE(chips_spi_board_info));
    if(status < 0){
        gf_debug(ERR_LOG,"Failed to register spi_board_info\n");
        return status;
    }
