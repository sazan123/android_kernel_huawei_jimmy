...
static int fwu_enter_flash_prog(void)
{
	...
	if (fwu->in_bl_mode)
		return 0;
#if 1	//comment out 20150703 to avoid write block timeout
	retval = rmi4_data->irq_enable(rmi4_data, true);//, true)
	if (retval < 0)
		return retval;
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	ret = synaptics_fw_updater(SynaFirmware);
	if(!ret)
	{
		fwu_get_device_config_id();
		sprintf(ver_buf,"fw_ver=0x%x,config_ver=0x%x.",fwu->rmi4_data->firmware_id,fwu->rmi4_data->config_id);
		strcat(vendor_name,ver_buf);
		printk(KERN_ERR "[kernel][synaptics]:%s synaptics_fw_updater succ.config ver=0x%x,fw ver=0x%x\n"
			 ,__func__,fwu->rmi4_data->config_id,fwu->rmi4_data->firmware_id);
	}
	else
	{
		printk(KERN_ERR "[kernel][synaptics]:%s synaptics_fw_updater fail.\n",__func__);
	}
	return;
}
#endif
