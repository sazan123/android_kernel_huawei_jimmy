...
char vendor_name[50]="oflim-s3603";
...
int tpd_i2c_read_data(struct synaptics_rmi4_data *rmi4_data,struct i2c_client *client,
			   unsigned short addr, unsigned char *data, unsigned short length)
 {
	...
	//printk(KERN_ERR "%s:client->addr=0x%x,addr=0x%x,length=%d\n",__func__,client->addr,addr,length);
	/* u16 old_flag; */
...
#endif
{
	...
	ret = of_get_synaptic_platform_data(&client->dev);
...
static const struct of_device_id synaptics_rmi4_dt_match[] = {
        {.compatible = "synaptics_dsx"},
        {},
};
MODULE_DEVICE_TABLE(of, synaptics_rmi4_dt_match);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 19))
	.driver.of_match_table = of_match_ptr(synaptics_rmi4_dt_match),
#endif
