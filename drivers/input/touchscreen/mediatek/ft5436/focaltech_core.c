...
static char ft5436_version[]="ft5436 ontim ver 1.0";
static char ft5436_vendor_name[50]="each-ft5436";
DEV_ATTR_DECLARE(touch_screen)
DEV_ATTR_DEFINE("version",ft5436_version)
DEV_ATTR_DEFINE("vendor",ft5436_vendor_name)
...
#else
            FTS_DEBUG("[B]P%d(%4d,%4d)[tm:%d] DOWN!", data->au8_finger_id[i], data->au16_x[i], data->au16_y[i],
                      data->area[i]);
#endif
        }
        else
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  //  fts_irq_disable();
    ...
    retval = fts_i2c_read_reg(client, FTS_REG_FW_VER, &tp_fw_ver);
    if(retval<0)
    {
	goto error_i2c;
    }
...
fts_i2c_read_reg(client, FTS_REG_FW_VER, &tp_fw_ver);
sprintf(ft5436_vendor_name,"yeji,ft5436,fw=0x%x\n",tp_fw_ver);
#endif
