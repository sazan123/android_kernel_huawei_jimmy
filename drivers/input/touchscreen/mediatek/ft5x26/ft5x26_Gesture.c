...
int fts_read_Gestruedata(void)
{
	...
	if (ret < 0) {
		FTS_DBG("%s read touchdata failed.\n", __func__);
		return ret;
	}
	...
	if (fts_updateinfo_curr.CHIP_ID == 0x54 || fts_updateinfo_curr.CHIP_ID == 0x58 ||
		...
		for (i = 0; i < pointnum; i++) {
			coordinate_x[i] = (((s16)buf[0 + (4 * i + 2)]) & 0x0F) << 8 |
										(((s16) buf[1 + (4 * i + 2)]) & 0xFF);
			coordinate_y[i] = (((s16)buf[2 + (4 * i + 2)]) & 0x0F) << 8 |
