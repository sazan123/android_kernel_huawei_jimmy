...
*******************************************************************************/
/*default master Device ID for ft_rw_iic_drv */
...
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		FTS_ERROR("Error %d adding LED%d", err, index);
}
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (err < 0) {
		FTS_ERROR("%s:ft_rw_iic_drv failed  error code=%d---\n", __func__, err);
		return err;
	}
	...
	if (!fts_rw_i2c_dev_tt) {
		...
		FTS_ERROR("%s:ft_rw_iic_drv failed\n", __func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		FTS_ERROR("%s:failed in creating class.\n", __func__);
		return -1;
	}
	/*create device node */
