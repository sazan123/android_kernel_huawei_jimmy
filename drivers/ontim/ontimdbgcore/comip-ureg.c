...
//		n_reg->val = readl(io_p2v(n_reg->paddr));
		n_reg->val = __raw_readl((n_reg->paddr));
	else {
...
//		writel(n_reg->val, io_p2v(n_reg->paddr));
		__raw_writel(n_reg->val, (n_reg->paddr));
	else {
		n_reg->paddr |= ISP_BASE;
...
{
	UREG_PRINT("ureg_init\n");
        
	return misc_register(&ureg_misc_dev);
}
