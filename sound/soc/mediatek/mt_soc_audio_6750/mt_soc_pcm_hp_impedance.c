...
/* static const int HpImpedancePhase1Step = 100; for alps-mp-n0.mp101.tc1sp*/
...
static void ApplyDctoDl(void)
{
	...
	for (value = 0; value < (HpImpedancePhase2AdcValue + HpImpedancePhase1Step);
		...
		/* usleep_range(1*250, 1*500); alps-mp-n0.mp101.tc1sp */
	...
	while (value > 0) {
		...
		/* apply to dram */
		FillDatatoDlmemory(Sramdata , Dl1_Playback_dma_buf->bytes , value);
		/* usleep_range(1*200, 1*400); alps-mp-n0.mp101.tc1sp */
		usleep_range(600, 800);
	}
#endif
}
