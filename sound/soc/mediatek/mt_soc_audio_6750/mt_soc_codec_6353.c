...
#ifdef CONFIG_MTK_SPEAKER   //add by ontim
...
/*delete audio disable before enable*/
#if 0
#if defined(CONFIG_MTK_LEGACY)
...
#else
		AudDrv_GPIO_EXTAMP_Select(false,0);
		//AudDrv_GPIO_EXTAMP2_Select(false);
#endif /*CONFIG_MTK_LEGACY*/

		/*udelay(1000); */
		usleep_range(1 * 1000, 20 * 1000);
#endif/*delete audio disable before enable*/
...
#else
		AudDrv_GPIO_EXTAMP_Select(true, 3);/*3 is the very ext amp mode*/
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_AnalogMic_Mode)) {
		pr_err("return -EINVAL\n");
#ifdef CONFIG_HUAWEI_DSM
        audio_dsm_report_num(DSM_AUDIO_CARD_LOAD_FAIL_ERROR_NO,DSM_AUDIO_MESG_SET_CODECCLK_FAIL);
#endif
		return -EINVAL;
	}
