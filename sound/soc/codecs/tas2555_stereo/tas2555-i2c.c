	...
	if (chn&channel_left) {
		if (pTAS2555->mnLCurrentBook == nBook) {
			if (pTAS2555->mnLCurrentPage != nPage) {
				nResult = tas2555_i2c_write_device(pTAS2555,
					pTAS2555->mnLAddr, TAS2555_BOOKCTL_PAGE, nPage);
				if (nResult >= 0)
					pTAS2555->mnLCurrentPage = nPage;
			}
		} else {
			nResult = tas2555_i2c_write_device(pTAS2555,
				pTAS2555->mnLAddr, TAS2555_BOOKCTL_PAGE, 0);
			if (nResult >= 0) {
				pTAS2555->mnLCurrentPage = 0;
				nResult = tas2555_i2c_write_device(pTAS2555,
					pTAS2555->mnLAddr, TAS2555_BOOKCTL_REG, nBook);
				pTAS2555->mnLCurrentBook = nBook;
				if (nPage != 0) {
					tas2555_i2c_write_device(pTAS2555,
						pTAS2555->mnLAddr, TAS2555_BOOKCTL_PAGE, nPage);
					pTAS2555->mnLCurrentPage = nPage;
				}
			}
	...
	if (chn&channel_right) {
		if (pTAS2555->mnRCurrentBook == nBook) {
			if (pTAS2555->mnRCurrentPage != nPage) {
				nResult = tas2555_i2c_write_device(pTAS2555,
					pTAS2555->mnRAddr, TAS2555_BOOKCTL_PAGE, nPage);
				if (nResult >= 0)
					pTAS2555->mnRCurrentPage = nPage;
			}
		} else {
			nResult = tas2555_i2c_write_device(pTAS2555,
				pTAS2555->mnRAddr, TAS2555_BOOKCTL_PAGE, 0);
			if (nResult >= 0) {
				pTAS2555->mnRCurrentPage = 0;
				nResult = tas2555_i2c_write_device(pTAS2555,
					pTAS2555->mnRAddr, TAS2555_BOOKCTL_REG, nBook);
	...
	unsigned int *pValue)
{
	...
	if (nResult >= 0) {
		if (chn == channel_left) {
			nResult = tas2555_i2c_read_device(pTAS2555,
				pTAS2555->mnLAddr, TAS2555_PAGE_REG(nRegister), &Value);
		} else if (chn == channel_right) {
			nResult = tas2555_i2c_read_device(pTAS2555,
				pTAS2555->mnRAddr, TAS2555_PAGE_REG(nRegister), &Value);
		} else {
	...
	unsigned int nValue)
{
	...
	if (nResult >= 0) {
		if (chn & channel_left)
			nResult = tas2555_i2c_write_device(pTAS2555,
				pTAS2555->mnLAddr, TAS2555_PAGE_REG(nRegister), nValue);
		if (chn & channel_right)
			nResult = tas2555_i2c_write_device(pTAS2555,
				pTAS2555->mnRAddr, TAS2555_PAGE_REG(nRegister), nValue);
	}
	...
	unsigned int nValue)
{
	...
	if (nResult >= 0) {
		if (chn&channel_left)
			tas2555_i2c_update_bits(pTAS2555, pTAS2555->mnLAddr,
				TAS2555_PAGE_REG(nRegister), nMask, nValue);

		if (chn&channel_right)
			tas2555_i2c_update_bits(pTAS2555, pTAS2555->mnRAddr,
				TAS2555_PAGE_REG(nRegister), nMask, nValue);
	}
	mutex_unlock(&pTAS2555->dev_lock);
	return nResult;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pTAS2555->mbFwLoaded = true;

	return 1;
}

static DEVICE_ATTR(fwload, 0220 , NULL,	store_fwload);

static struct device_attribute *sysfs_device_attr_fwload = {
		&dev_attr_fwload,
		};
#endif
...
#ifdef FASTBOOT_PLATFORM
		nResult = request_firmware_nowait(THIS_MODULE, 1, TAS2555_FW_NAME,
			pTAS2555->dev, GFP_KERNEL, pTAS2555, tas2555_fw_ready);
#else
		/* create sysfs variables */
		nResult = device_create_file(&pClient->dev,
					 sysfs_device_attr_fwload);
		if (nResult < 0)
			dev_info(pTAS2555->dev, "Error to Create fwload device attribute");
#endif
