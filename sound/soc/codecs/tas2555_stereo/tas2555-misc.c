			...
			if (count == 1) {
				...
				if (p_kBuf != NULL) {
					ret = pTAS2555->bulk_read(pTAS2555,
						pTAS2555->mnCurrentChannel,
						pTAS2555->mnCurrentReg, p_kBuf, count);
					if (0 > ret)
			...
			if (count == CONFIGURATION_BUF_SIZE) {
				...
				if (p_kBuf != NULL) {
					TConfiguration *pConfiguration =
						&(pTAS2555->mpFirmware->
						mpConfigurations[pTAS2555->mnCurrentConfiguration]);
	...
	case TIAUDIO_CMD_BITRATE:
		{
			...
			if (count == 1) {
				...
				tas2555_get_bit_rate(pTAS2555,
					pTAS2555->mnCurrentChannel, &bitRate);
	...
	case TIAUDIO_CMD_DACVOLUME:
		{
			...
			if (count == 1) {
				...
				ret = tas2555_get_DAC_gain(pTAS2555,
					pTAS2555->mnCurrentChannel, &volume);
				if (ret >= 0) {
...
static ssize_t tas2555_file_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	...
	case TIAUDIO_CMD_REG_READ:
		if (count == 6) {
			pTAS2555->mnCurrentChannel = p_kBuf[1];
			pTAS2555->mnCurrentReg = ((unsigned int)p_kBuf[2] << 24) +
				((unsigned int)p_kBuf[3] << 16) +
				((unsigned int)p_kBuf[4] << 8) +
				(unsigned int)p_kBuf[5];
			if (g_logEnable) {
	...
	case TIAUDIO_CMD_BITRATE:
		{
			...
			if (count == 3) {
				...
				tas2555_set_bit_rate(pTAS2555,
					pTAS2555->mnCurrentChannel, p_kBuf[2]);
			}
		}
		break;
	case TIAUDIO_CMD_DACVOLUME:
...
static long tas2555_file_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SMARTPA_SPK_SET_BITRATE:
		tas2555_set_bit_rate(pTAS2555, channel_both, arg);
		break;
	}
