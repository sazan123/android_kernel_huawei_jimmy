...
#define IIC_DMA_MAX_BUFFER_SIZE 250
...
static int rt_i2c_master_send(const struct i2c_client *client, const char *buf, int count)
{
	...
	if (count > 8) {
		msg.ext_flag |= (I2C_ENEXT_FLAG | I2C_DMA_FLAG);
		memcpy(chip->rt_gpDMABuf_va, buf, count);
		msg.buf = (char *)(uintptr_t)chip->rt_gpDMABuf_pa;
	} else
		msg.buf = (char *)buf;
...
static int rt_i2c_master_recv(const struct i2c_client *client, char *buf, int count)
{
	struct rt5509_chip *chip = i2c_get_clientdata(client);
	struct i2c_adapter *adap = client->adapter;
	struct i2c_msg msg;
	...
	ret = i2c_transfer(adap, &msg, 1);
	if (count > 8)
		memcpy(buf, chip->rt_gpDMABuf_va, count);
	...
	void *client, u32 reg, int bytes, void *dest)
{
	...
	ret = rt_i2c_master_send(client, (const char *)&reg, 1);
	if (ret < 0)
		return ret;
	return rt_i2c_master_recv(client, dest, bytes);
	...
	int bytes, const void *src)
{
	...
	tmp[0] = (u8)reg;
	memcpy(tmp + 1, src, bytes);
	return rt_i2c_master_send(client, tmp, bytes + 1);
#endif /* #if RT5509_SIMULATE_DEVICE */
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	chip->rt_gpDMABuf_va = dma_alloc_coherent(&client->dev, IIC_DMA_MAX_BUFFER_SIZE,
						  &chip->rt_gpDMABuf_pa, GFP_KERNEL);
	if (chip->rt_gpDMABuf_va == NULL) {
		dev_err(&client->dev, "dma_alloc_coherent error\n");
		goto err_sw_reset;
	}
	memset(chip->rt_gpDMABuf_va, 0, IIC_DMA_MAX_BUFFER_SIZE);
...
static int rt5509_i2c_remove(struct i2c_client *client)
{
	...
	_rt5509_power_on(chip, false);
	if (chip->rt_gpDMABuf_va != NULL) {
		dma_free_coherent(&client->dev, IIC_DMA_MAX_BUFFER_SIZE,
				  chip->rt_gpDMABuf_va, chip->rt_gpDMABuf_pa);
		chip->rt_gpDMABuf_va = NULL;
		chip->rt_gpDMABuf_pa = 0;
	}
#if RT5509_SIMULATE_DEVICE
	devm_kfree(chip->dev, chip->sim);
#endif /* #if RT5509_SIMULATE_DEVICE */
