...
#+modify by hzb
    ifneq ($(wildcard $(srctree)/drivers/misc/mediatek/mach/$(MTK_PLATFORM)/common/dct/$(PRIVATE_CUSTOM_KERNEL_DCT)/codegen.dws),)
      DRVGEN_PATH := drivers/misc/mediatek/mach/$(MTK_PLATFORM)/common/dct/$(PRIVATE_CUSTOM_KERNEL_DCT)
      DWS_FILE := $(srctree)/$(DRVGEN_PATH)/codegen.dws
    else
...
$(warning DWS_FILE == $(DWS_FILE), DRVGEN_PATH ==$(DRVGEN_PATH),CUSTOM_KERNEL_DCT==$(CUSTOM_KERNEL_DCT))
...
#+modify by hzb
ifneq ($(wildcard $(srctree)/tools/dct_tools/DrvGen),)
    DRVGEN_TOOL := $(srctree)/tools/dct_tools/DrvGen
else
