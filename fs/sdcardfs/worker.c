...
#define override_cred_set_key_internal(new, owner, xxx) \
{\
    if(new->xxx != owner->xxx) {\
        /*New key is not same as that of owner. Deferrence new key and make sure referrence owner key*/\
        key_put(new->xxx);\
        key_get(owner->xxx);\
    }\
    else {\
        /*Both keys are same. Do nothing since reference has been done in prepare_creds()*/\
    }\
    new->xxx = owner->xxx;\
}
	...
	const struct cred *owner_cred)
{
    ...
    override_cred_set_key_internal(cred, owner_cred, session_keyring);
    override_cred_set_key_internal(cred, owner_cred, process_keyring);
    override_cred_set_key_internal(cred, owner_cred, thread_keyring);
    override_cred_set_key_internal(cred, owner_cred, request_key_auth);
#endif
